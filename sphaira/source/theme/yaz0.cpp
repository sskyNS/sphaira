...
std::vector<u8> Decompress(const std::vector<u8>& Data) {
    ...
    const u32 leng = (u32)(Data[4] << 24 | Data[5] << 16 | Data[6] << 8 | Data[7]);
    ...
    while (true) {
        ...
        for (int i = 0; i < 8; i++) {
            if ((header & 0x80) != 0) {
                Result.at(dstoffs++) = Data.at(Offs++);
            } else {
                const u8 b = Data.at(Offs++);
                const int offs = ((b & 0xF) << 8 | Data.at(Offs++)) + 1;
                int length = (b >> 4) + 2;
                if (length == 2) {
                    length = Data.at(Offs++) + 0x12;
                }
...
std::vector<u8> Compress(const std::vector<u8>& Data, int level, int reserved1, int reserved2) {
    level = std::clamp(level, 1, 9);
    const int maxBackLevel = (int)(0x10e0 * (level / 9.0) - 0x0e0);

    std::vector<u8> result(Data.size() + Data.size() / 8 + 0x10);
            ...
            if (Offs) {
                ...
                while (maxbackptr < ptr) {
                    if (ptr[0] == sourceptr[0] && ptr[1] == sourceptr[1] && ptr[2] == sourceptr[2]) {
                        tmpnr = 3;
                        while (tmpnr < maxnum && ptr[tmpnr] == sourceptr[tmpnr]) {
                            tmpnr++;
                        }
                        if (tmpnr > nr) {
